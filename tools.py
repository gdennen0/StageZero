    ...
    try:
        if song_object is not None:
            ...
            tempo, beats = librosa.beat.beat_track(
                y=song_data, sr=sample_rate, hop_length=hop_length
            )

            print(
                f"[TOOLS][estimate_bpm] song name: {song_name}, sample rate {sample_rate}, tempo {tempo}"
            )
            return tempo, beats
        else:
...
def detect_onsets(song_object=None, song_data=None, sample_rate=None):
    ...
    onsets = librosa.onset.onset_detect(
        y=song_data, sr=sample_rate, hop_length=hop_length, units="frames"
    )
