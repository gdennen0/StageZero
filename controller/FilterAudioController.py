...
from view import SongDataPreviewWindow
...
class FilterAudioController:
    ...
    def connect_signals(self):
        self.filter_audio_window.filter_list_widget.itemSelectionChanged.connect(
            self.display_filter_properties
        )
        self.filter_audio_window.apply_filter_to_song.clicked.connect(
            self.apply_filter_to_audio
        )
        self.filter_audio_window.preview_filtered_data.clicked.connect(
            self.preview_filtered_data
        )
    ...
    def display_filter_properties(self):
        ...
        if selected_item:
            ...
            self.filter_audio_window.filter_name_label.setText(
                f"Filter Name: {properties.get('filter_name', '')}"
            )
            self.filter_audio_window.filter_type_label.setText(
                f"Filter Type: {properties.get('filter_type', '')}"
            )
            self.filter_audio_window.filter_cutoff_frequency_label.setText(
                f"Cutoff Frequency: {properties.get('cutoff_frequency', '')}"
            )
